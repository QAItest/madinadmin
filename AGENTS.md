# Madin'Admin — Contrat d'orchestration Codex
...
|-------|--------|------|-------------------|-------------------|
| **Diagnostiqueur** | Codex-opus-4-7 | Éligibilité et cadrage | `porteurs/{porteur}/profil.md` + description projet | `diagnostics/{date}-{porteur}-eligibilite.md` |
| **Monteur** | Codex-sonnet-4-5 | Rédaction des sections du dossier | Diagnostic validé + profil porteur | `dossiers/{date}-{porteur}-{dispositif}/sections/*.md` |
| **Documentaliste** | Codex-sonnet-4-5 | Checklist pièces justificatives | Profil porteur + dispositif | `dossiers/{dossier}/checklist.md` + `pieces-manquantes.md` |
| **Contrôleur** | Codex-opus-4-7 | Audit de conformité | Dossier complet monté | `controles/{date}-{dossier}-rapport-conformite.md` |
| **Suiveur** | Codex-sonnet-4-5 | Suivi post-dépôt | Journal des échanges + échéances | `suivi/{dossier}/journal.md` + `echeances.md` |
| **Archiviste** | Codex-opus-4-7 | Rapports et dossier d'audit | Dossier clos | `rapports/{periode}-{dossier}.md` + `archives/{dossier}/` |
| **Veilleur** | Codex-opus-4-7 | Veille appels à projets | Base porteurs + appels actifs | Notifications dans `porteurs/{porteur}/veille.md` |
| **OCR** | Codex-sonnet-4-5 | Extraction documents | Fichiers uploadés (PDF, image) | Données structurées JSON |
| **Courrier** | Codex-sonnet-4-5 | Rédaction courriers administratifs | Demandes pièces complémentaires | Lettres formatées dans `suivi/{dossier}/courriers/` |
...
```
MadinAdmin/
├── AGENTS.md                          # Ce fichier — contrat d'orchestration
├── .Codex/
...
### Commandes de prospection porteur *(source : ai-sales-team-Codex + ai-marketing-skills)*
...
### Commandes d'amélioration & investigation *(source : oh-my-Codex)*
